...
export function getAvailableTimeSlots(date: string, serviceDuration = 60): TimeSlot[] {
  ...
  const dayBookings = bookings.filter((b) => b.date === date && b.status !== "cancelado")

  return allSlots.map((time) => {
    const available = isTimeSlotAvailable(time, serviceDuration, dayBookings, daySchedule)
    return { time, available }
  })
}

function isTimeSlotAvailable(time: string, duration: number, bookings: Booking[], daySchedule: DaySchedule): boolean {
  ...
  if (startMinutes < lunchEndMinutes && endMinutes > lunchStartMinutes) return false
...
export function markNotificationsSent(bookingId: string, type: "email" | "whatsapp"): void {
  const bookings = getBookingsFromStorage()
  const updated = bookings.map((b) =>
    b.id === bookingId ? { ...b, notificationsSent: { ...b.notificationsSent, [type]: true } } : b,
  )
  localStorage.setItem("bookings", JSON.stringify(updated))
}
...
async function sendEmailNotification(booking: Booking, settings: NotificationSettings): Promise<void> {
...
async function sendWhatsAppNotification(booking: Booking, settings: NotificationSettings): Promise<void> {

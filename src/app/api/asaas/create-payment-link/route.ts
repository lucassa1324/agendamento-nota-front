...
export async function POST(req: Request) {
  try {
    ...
    if (!subscriptionId) {
      const createSubscriptionResponse = await fetch(`${ASAAS_API_URL}/subscriptions`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          access_token: ASAAS_API_KEY,
        ...
        body: JSON.stringify({
          ...
          value: 49.99,
          nextDueDate: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Vence amanhã
          cycle: "MONTHLY",
    ...
    return NextResponse.json(
      { error: "Não foi possível gerar o link de pagamento neste momento. Tente novamente em instantes." },
      { status: 500 }
    );

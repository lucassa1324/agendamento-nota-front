...
export async function GET(req: Request) {
  try {
    ...
    const subsRes = await fetch(
      `${ASAAS_API_URL}/subscriptions?customer=${customer.id}`,
      {
        headers: { access_token: ASAAS_API_KEY },
      },
    );
    ...
    const paymentsRes = await fetch(
      `${ASAAS_API_URL}/payments?customer=${customer.id}&limit=5`,
      {
        headers: { access_token: ASAAS_API_KEY },
      },
    );
    ...
    return NextResponse.json({
      ...
      lastPaymentDate:
        lastPayment?.paymentDate || lastPayment?.clientPaymentDate || null,
      nextInvoiceDate:

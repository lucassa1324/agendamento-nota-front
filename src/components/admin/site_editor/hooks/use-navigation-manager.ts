...
export function useNavigationManager(
  ...
  const activeSectionData = Object.values(sections)
    .flat()
    .find((s: SectionItem) => s.id === activeSection);
    ...
    (pageId: string) => {
      setExpandedPages((prev: string[]) =>
        prev.includes(pageId)
          ? prev.filter((id: string) => id !== pageId)
          : [...prev, pageId],
      );
    ...
    if (iframe) {
      const handleLoad = () => {
        ...
        if (iframe.contentWindow) {
          ...
          const sectionId =
            activeSection === "typography" ? null : activeSection;
